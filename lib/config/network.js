...
function sanitize(region, config) {
  if (!config.network) {
    ...
    var normalizedConfig = sanitizeNetwork(region, config);
...
function sanitizeNetwork(region, config) {
  return _.extend(
    Object(),
    config,
    {
      network: {
        ...
        nat: sanitizeNat(region, config),
        subnets: sanitizeSubnets(region, config),
      }
    }
  );
}

function sanitizeNat(region, config) {
  return _.extend(
    Object(),
    {
      type: 't2.micro',
...
function sanitizeSubnets(region, config) {
