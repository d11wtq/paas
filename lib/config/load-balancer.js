...
function portForwards(config) {
  var mapping = (config['load-balancer'] || Object()).ports
    , wrap = {
        'http': 'http',
        'https': 'http',
        'tcp': 'tcp',
...
function validateLoadBalancerPorts(config) {
  validateLoadBalancerPortsBoundOnce(config);
  validateLoadBalancerPortsInRange(config);
}

function validateLoadBalancerPortsBoundOnce(config) {
  portForwards(config)
    .reduce(
      function(seen, forward){
        if (seen[forward.port]) {
          throw new Error(
            util.format(
              'load-balancer.ports[%d]: bound multiple times',
              forward.port
            )
          );
        }

        return _.set(seen, [forward.port], true);
      },
      Object()
    );
}

function validateLoadBalancerPortsInRange(config) {
...
function validateInstancePorts(config) {
  validateInstanceProtocols(config);
  validateInstancePortsPublished(config);
}

function validateInstanceProtocols(config) {
  portForwards(config)
    .reduce(
      function(seen, forward){
        if (seen[forward.destination.port]) {
          if (seen[forward.destination.port] != forward.destination.type) {
            throw new Error(
              util.format(
                'load-balancer.ports[%d]: cannot forward to both %d/%s and %d/%s',
                forward.port,
                forward.destination.port,
                seen[forward.destination.port],
                forward.destination.port,
...
function validateInstancePortsPublished(config) {
...
function validateCertificate(config) {
  portForwards(config).forEach(function(forward){
    if (isSSL(forward) && ! forward.certificate) {
      throw new Error(
        util.format(
          'load-balancer.ports[%d/%s]: certificate required on load-balancer',
          forward.port,
          forward.type
        )
      );
