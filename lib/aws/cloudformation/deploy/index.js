 ...
 */
function Deployment(service, name, config) {
  var self = this
  ...
  this.perform = function() {
    apply(function(err, res){
      ...
      if (err) {
        ...
        concurrently(events(service, res.StackId))
          .on('evt', function(e){
            ...
            if (e.PhysicalResourceId && e.LogicalResourceId == asgResource) {
              asgCreated = true;
              concurrently(monitor(service, e.PhysicalResourceId))
 ...
 */
function deploy(service, name, config) {
  var deployment = new Deployment(service, name, config);
  deployment.perform();
  return deployment;
}
